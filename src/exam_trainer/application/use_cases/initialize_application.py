...
__all__ = ["InitializeApplication", "WorkspaceError"]
...
class InitializeApplication:
    ...
    def get_startup_state(self) -> StartupState:
        ...
        workspace = Workspace.from_path(Path(saved_path).expanduser())
    ...
    def configure_workspace(self, selected_directory: Path | str) -> Workspace:
        workspace = Workspace.from_path(Path(selected_directory).expanduser())
